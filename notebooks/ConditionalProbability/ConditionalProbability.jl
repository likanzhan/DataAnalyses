...
# ╔═╡ 5634fa35-67e9-4b34-bf68-f1377635d7a0
md"""
## Other Analyses
"""
...
# ╔═╡ 8755a8af-47a4-4582-83e5-8e0adb28c34d
fm2 = fit(MixedModel, @formula(rate ~ C1GvA0 + (1 | participant)), NNN)

# ╔═╡ f708db3c-da85-4251-baf2-5e7333f84333
let
	fig = Figure()
	ax = Axis(fig[1, 1], 
		xlabel = L"P(C|\neg A)", ylabel = L"P (\text{if A, then C})")
	scatter!(ax, NNN.C1GvA0, NNN.rate, color = (:blue, 0.1))
	ablines!(ax, [0], [1], color = :green, 
		label = L"P(C|\neg A) = P (\text{if A, then C})")
	ablines!(ax, coef(fm2)..., color = :red, 
		label = L"\text{Actual Results}")
	axislegend(position = :lt)
	fig
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "c4306dc95bd5cafabe193b420cb13028168844eb"
