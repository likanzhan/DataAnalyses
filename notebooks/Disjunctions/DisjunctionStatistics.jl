...
# ╔═╡ e24fda80-3a57-4cab-a321-4f3b45c1fb0e
using Gadfly
...
# subset!(df, :participant => ByRow(!=("S05")))

# ╔═╡ 82876544-590d-44ed-aa7c-eeb6ccbf1179
set_default_plot_size(15cm, 25cm)

# ╔═╡ c4dd8f54-06f1-4070-bfb2-10a0aae95d2d
plot(df, x = :time, y = :AFavg, color = :connective, 	
	xgroup = :pause, ygroup = :participant, 
	Geom.subplot_grid(Geom.line, 
	Coord.cartesian(xmin = -200, xmax = 1000, ymin=-40, ymax=40))
)

# ╔═╡ 4bf4b72d-bb3b-4065-bc1e-147f1bc24c58
...
gptdf = groupby(df, [:pause, :time]);

# ╔═╡ 6c686b26-aa9c-4feb-aab4-426dda517ae3
pvalue(x) = (coeftable(fit(LinearModel, @formula(AFavg ~ connective), x)).cols)[4][2];

# ╔═╡ f40efe59-e82a-4d05-a48e-bffdceb9b304
res1 = insertcols!(DataFrame(keys(gptdf)), :pvals => [pvalue(x) for x in gptdf]);

# ╔═╡ 6ca2edfc-50e4-4df0-bfbf-0df8aeed3391
sigres1 = subset(res1, :pvals => ByRow(<=(.05)));

# ╔═╡ 46ab15ef-7f57-4a86-8f68-ca09896b7827
pptdf = combine(groupby(df, [:connective, :pause, :time]), :AFavg => mean => :AFavg);

# ╔═╡ 4a135fbb-dd1f-417b-aa57-f19e4fb26841
pptdfp = innerjoin(pptdf, res1, on = [:pause, :time]);

# ╔═╡ 4e00304b-596b-47f3-ae4e-572068994f8d
# subset(pptdfp, :pvals => ByRow(<=(.05)))

# ╔═╡ 6f7bb93f-39b4-44da-a14c-4d4a6b62b5aa
plot(pptdfp, x = :time, y = :AFavg, color = :connective, xgroup = :pause, 
	Geom.subplot_grid(
		Geom.line, 
		Coord.cartesian(xmin = -200, xmax = 1000, ymin=-10, ymax=5))
)

# ╔═╡ 3796f916-8f61-44ae-8ec2-0091094c9415
plot(sigres1, x = :time, y = :time, xgroup = :pause, 
	Geom.subplot_grid(Geom.point,
		Coord.cartesian(xmin = -200, xmax = 1000, ymin = 250, ymax = 500))
	)
...
tdf = combine(groupby(df, [:participant, :connective, :time]), 
	:AFavg => mean => :AFavg);

# ╔═╡ c84cc5a6-dde8-4dba-bd2b-ea41b42b9ca0
gtdf = groupby(tdf, :time);

# ╔═╡ af270507-fd47-46e8-9b53-f2be6c71d1a6
res2 = insertcols!(DataFrame(keys(gtdf)), :pvals => [pvalue(x) for x in gtdf]);

# ╔═╡ a3197447-9322-4f67-b23f-598f468a033e
sigres2 = subset(res2, :pvals => ByRow(<=(.05)));

# ╔═╡ 0381bec0-486f-42e2-bc56-8bba9d55a84e
ptdf = combine(groupby(tdf, [:connective, :time]), :AFavg => mean => :AFavg);

# ╔═╡ 2ee645ee-bf95-41b5-ae81-17bf79c4729a
ptdfp = innerjoin(ptdf, res2, on = :time);

# ╔═╡ c86cf7ce-4780-495e-bfd8-3a4529181dac
plot(ptdfp, x = :time, y = :AFavg, color = :connective, Geom.line,
	Coord.cartesian(xmin = -200, xmax = 1000, ymin=-10, ymax=5))

# ╔═╡ b648f5c5-3aad-4424-b61a-2ba0c1c25ae3
plot(sigres2, x = :time, y = :time, Geom.point,
	 Coord.cartesian(xmin = -200, xmax = 1000, ymin = 250, ymax = 500)
	)
